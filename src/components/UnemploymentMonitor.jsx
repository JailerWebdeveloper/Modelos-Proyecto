...
const UnemploymentMonitor = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          'https://api.worldbank.org/v2/countries/CO/indicators/SL.UEM.TOTL.ZS?format=json'
        );
  ...
  return (
    <div>
      <h2 className='text-center text-xl'>Monitor de desempleo</h2>
      {data.length > 0 ? (
        <Chart
          width={'100%'}
          height={'400px'}
          ...
          data={[['Year', 'Unemployment'], ...data.map(item => [item.date, item.value])]}
          options={{
...
export default UnemploymentMonitor;
