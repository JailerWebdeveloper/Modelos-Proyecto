...
const InflationMonitor = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          'https://api.worldbank.org/v2/countries/CO/indicators/FP.CPI.TOTL.ZG?format=json'
        );
  ...
  return (
    <div>
      <h2 className='text-center text-xl'>Monitor de inflacion</h2>
      {data.length > 0 ? (
        <Chart
          width={'100%'}
          height={'400px'}
          ...
          data={[['Year', 'Inflation'], ...data.map(item => [item.date, item.value])]}
          options={{
